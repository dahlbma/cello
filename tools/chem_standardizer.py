...
try:
    ...
    try:
        # fallback when running as a script from repo root
        from tools.exclude_flag import exclude_flag
    except Exception:
        # last resort: top-level import (if exclude_flag is on PYTHONPATH)
        try:
            from exclude_flag import exclude_flag
        except Exception:
            # define a stub that never excludes (keeps behavior safe)
            def exclude_flag(m, includeRDKitSanitization=True):
                return False
import rdkit
import math
...
def standardize_mol(m, check_exclusion=True, sanitize=True):
    ...
    if not exclude:
        ...
        if sanitize:
            # Chem.SanitizeMol mutates the molecule in-place and returns a
            # SanitizeFlags bitmask. Capture the flags so callers can log or
            # react to which sanitize operations were applied.
            flags = Chem.SanitizeMol(m)
            # only record flags when something actually happened (non-zero)
            try:
                if int(flags) != 0:
                    m.SetProp("_sanitize_flags", str(int(flags)))
            except Exception:
                # ignore failures to set properties
                pass
            try:
                if int(flags) != 0:
                    m._sanitize_flags = flags
            except Exception:
                pass
...
# Command-line SDF standardization helper
import argparse
import sys


def standardize_sdf(input_sdf, output_sdf, *, check_exclusion=True, sanitize=False, skip_errors=True, log_sanitize_flags=False):
    """Read molecules from an input SDF, standardize each, and write to output SDF.

    Returns a tuple: (n_in, n_written, errors)
    """
    ...
    n_written = 0
    errors = []
    for idx, m in enumerate(supplier, start=1):
        n_in += 1
        if m is None:
            msg = f"Molecule {idx} could not be read"
            if skip_errors:
        ...
        try:
            std = standardize_mol(m, check_exclusion=check_exclusion, sanitize=sanitize)
            if std is None:
                # skip None results
                errors.append((idx, 'standardization returned None'))
                continue
            if sanitize and log_sanitize_flags:
                # attempt to get flags from molecule property or attribute
                flags = None
                try:
                    if std.HasProp('_sanitize_flags'):
                        flags = int(std.GetProp('_sanitize_flags'))
                except Exception:
                    flags = None
                if flags is None:
                    try:
                        flags = int(getattr(std, '_sanitize_flags', None))
                    except Exception:
                        flags = None
                # print to stderr (keeps stdout for normal CLI messages)
                if flags is not None and flags != 0:
                    print(f"molecule {idx} sanitize_flags={flags}", file=sys.stderr)
            writer.write(std)
            n_written += 1
            ...
            if skip_errors:
                ...
                raise
    writer.close()
    return n_in, n_written, errors


def _cli():
    p = argparse.ArgumentParser(description='Standardize molecules in an SDF using chem_standardizer')
    p.add_argument('input', help='Input SDF path')
    p.add_argument('output', help='Output SDF path')
    p.add_argument('--no-exclusion', dest='check_exclusion', action='store_false', help='Do not run exclusion checks')
    p.add_argument('--sanitize', dest='sanitize', action='store_true', help='Sanitize molecules after standardization')
    p.add_argument('--log-sanitize-flags', dest='log_sanitize_flags', action='store_true', help='Log sanitize flags to stderr for each molecule')
    p.add_argument('--no-skip-errors', dest='skip_errors', action='store_false', help='Raise on molecule errors instead of skipping')
    args = p.parse_args()
    n_in, n_written, errors = standardize_sdf(
        args.input,
        args.output,
        check_exclusion=args.check_exclusion,
        sanitize=args.sanitize,
        skip_errors=args.skip_errors,
        log_sanitize_flags=args.log_sanitize_flags,
    )
    print(f"Read {n_in} molecules, wrote {n_written} standardized molecules to {args.output}")
    if errors:
        print(f"Encountered {len(errors)} errors (showing up to 10):")
        for e in errors[:10]:
            print(e)
